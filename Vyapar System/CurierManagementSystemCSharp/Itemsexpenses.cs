...
{
    public partial class Itemsexpenses : Form
    {
        private string watermarkText1 = "SEARCH";

        public Itemsexpenses()
        {
            InitializeComponent();
        }

        private void Itemsexpenses_Load(object sender, EventArgs e)
        {
        ...
        private void searchData(string valueToSearch)
        {
            try
            {
                string query = "SELECT * FROM expenses_real WHERE CONCAT( ID, Date, Item, amount, total) LIKE @valueToSearch";
        ...
        private void gettingitemsexpenses()
        {
            try
            {


                string query = "SELECT Item, amount FROM expenses_real";
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                try
                {
                    ...
                    string query = "SELECT ID, Date, Item, amount, total FROM expenses_real WHERE Item = @Item";
                    ...
                    using (SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\courier.mdf;Integrated Security=True;"))
                    {
                        ...
                        string query2 = "SELECT total FROM expenses_real WHERE Item = @Item";
                        //SqlCommand command = new SqlCommand(query3, con);

                        SqlCommand cmd2 = new SqlCommand(query2, con);
                        cmd2.Parameters.AddWithValue("@Item", selectedItemName);
                        ...
                        SqlCommand cmd = new SqlCommand(query, con);
                        cmd.Parameters.AddWithValue("@Item", selectedItemName);
                        SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                        DataTable dt = new DataTable();
                        adapter.Fill(dt);
        ...
        private void refreshbtn_Click(object sender, EventArgs e)
        {
            try
            {
                string query = "SELECT ID, Date, Item, amount, total FROM expenses_real";
                using (SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=|DataDirectory|\courier.mdf;Integrated Security=True;"))
                {
                    con.Open();
                    SqlCommand cmd = new SqlCommand(query, con);
